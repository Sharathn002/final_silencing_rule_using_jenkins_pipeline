...
def create_silencing_rule(curr_time_in_millisec,cluster_name,api_token,duration):
    duration=float(duration.split('h')[0])
...
def main():
    ...
    if args.function.lower() == 'create':
        create_silencing_rule(curr_time_in_millisec,args.UPDATE_CLUSTER, args.API_TOKEN,args.TIMEOUT)
    elif args.function.lower() == 'delete':
        delete_silencing_rule(args.UPDATE_CLUSTER,args.API_TOKEN)
    else:
